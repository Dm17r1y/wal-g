...
type PgbackrestLastestBackupSelector struct{
	Stanza string
}

type PgbackrestNamedBackupSelector struct {
	BackupName string
	Stanza string
}

func (selector PgbackrestLastestBackupSelector) Select(folder storage.Folder) (string, error) {
...
func (selector PgbackrestNamedBackupSelector) Select(folder storage.Folder) (string, error) {
	backupList, err := GetBackupList(folder, selector.Stanza)
	if err != nil {
		return "", err
	}
...
func NewBackupSelector(backupName string, stanza string) internal.BackupSelector {
	if backupName == internal.LatestString {
		...
		return PgbackrestLastestBackupSelector{Stanza: stanza}
	}

	tracelog.InfoLogger.Printf("Selecting the backup with name %s...\n", backupName)
	return PgbackrestNamedBackupSelector{BackupName: backupName, Stanza: stanza}
}
